...
/**
 * @typedef OidcPluginOpts
 * @property {Object.<String,String>} authQuery
 * @property {Object.<String,String>} claims
...
OIDC.create = function (libauth, libOpts) {
  /**
   * @param {OidcPluginOpts} pluginOpts
   */
  return function _oidc(pluginOpts) {
    ...
    async function generateAuthUrl(req, res, next) {
      ...
      let max = -1 + Math.pow(2, 32);
      let rnd = Crypto.randomInt(max);
      /* { clientId, scope } */
      let query = Object.assign(pluginOpts.authQuery, req.query, {
        redirect_uri: OIDC.getRedirectUri(libauth, libOpts, pluginOpts, req),
        state: rnd.toString(16),
        response_type: "code",
      });
 ...
 * @param {String} str
 */
OIDC.startsWithProto = function (str) {
 ...
 * @param {String} str
 */
OIDC.stripTrailingSlash = function (str) {
  if (!str.endsWith("/")) {
    return str;
  }
 ...
 */
OIDC.getRedirectUri = function (libauth, libOpts, pluginOpts, req) {
  // TODO we could allow arbitrary redirectUri,
  // but we'd need to also check to make sure it's allowed

  // ex: https://app.example.com/api/authn/session/oidc/accounts.google.com/redirect
  let redirectUri = libauth.get(req, "redirectUri") || pluginOpts.redirectUri;
  if (!redirectUri) {
    let pathname = req.originalUrl || req.url;
    let url = new URL(`https://example.com${pathname}`);
    redirectUri = url.pathname;
  }

  if (!OIDC.startsWithProto(redirectUri)) {
    let redirectHost = libauth.get(req, "redirectHost") || libOpts.issuer;
    if (!OIDC.startsWithProto(redirectHost)) {
      redirectHost = `https://${redirectHost}`;
    }
    redirectHost = OIDC.stripTrailingSlash(redirectHost);
    redirectUri = `${redirectHost}${redirectUri}`;
  }
