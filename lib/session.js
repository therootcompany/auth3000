...
LibAuth.create = function (issuer, PRIVATE_KEY, myOptions) {
  ...
  let libOpts = {
    ...
    cookies: myOptions.cookies || {
      path: myOptions.cookiePath || "/api/session/",
    },
  };
  ...
  let cookieDefaults = Object.assign(
    {
      signed: true,
      path: "/api/session/",
      httpOnly: true,
  ...
  if (!cookieDefaults.path) {
    throw new Error(`[libauth] missing 'opts.cookies.path'`);
  }
  if (!cookieDefaults.path.startsWith("/")) {
    throw new Error(
      `[libauth] 'opts.cookies.path' should start with leading '/'`,
    );
  }
  if (!cookieDefaults.path.endsWith("/")) {
    throw new Error(
      `[libauth] 'opts.cookies.path' should end with trailing '/'`,
    );
  }
  if (cookieDefaults.path.length < 3) {
    throw new Error(
      `[libauth] 'opts.cookies.path' should be scoped to a path, such as '/api/session/'`,
    );
  }
  ...
  function _redirectWithQuery(redirectUri) {
    ...
    async function _redirecter(req, res, next) {
      ...
      */
      ...
      let errResp = libauth.get(req, "oidcError");
      let err = libauth.get(req, "error");
      if (errResp || err) {
        query.error = errResp?.error || err?.code || "";
        query.error_description =
          errResp?.error_description || err?.message || "";
        query.error_uri = errResp?.error_uri || err?.url || "";
      }
